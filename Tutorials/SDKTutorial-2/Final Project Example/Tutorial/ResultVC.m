...
-(NSDictionary *)cfgFromJSONFileNamed:(NSString *)name
{
    ...
    NSError *error;
    NSDictionary *jsonD = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingAllowFragments error:&error];
    if (error) {
        [self epicFailWithError:error];
        return nil;
    }
    
    NSMutableDictionary *cfg = [NSMutableDictionary dictionaryWithDictionary:jsonD];
    
    // Add output info during run time
    cfg[hcrOutputsInfo] = @[
                            @{
                                hcrOutputType:hcrVideo,
                                hcrRelativePath:[NSString stringWithFormat:@"output_%@.mov", NSUUID.UUID.UUIDString]
                                }
                            ];
...
-(void)renderAnExample
{
    HCRender *renderer = [self rendererWithCFGNamed:@"renderCFGExample2"];
